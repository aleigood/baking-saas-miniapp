<template>
	<view class="bar-chart-container">
		<view v-if="!chartData || chartData.length === 0" class="chart-placeholder">暂无统计数据</view>
		<!-- [核心修改] 恢复为横向条形图布局 -->
		<view v-else class="chart-content">
			<view v-for="(item, index) in chartData" :key="item.name" class="bar-item">
				...
				<view class="bar-wrapper">
					<view class="bar" :style="{ width: getBarWidth(item.value), background: getBarColor(index) }"></view>
...
// 预定义的颜色列表
const colors = ['#8c5a3b', '#d4a373', '#a98467', '#c2956f', '#e6b89c'];
