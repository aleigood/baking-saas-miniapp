	...
	<view class="page-wrapper">
		...
		<DetailPageLayout @scroll="handleScroll">
			<view class="page-content page-content-with-fab">
				<view class="card">
					<FormItem label="配方名称">
						<input class="input-field" v-model="form.name" placeholder="例如：波兰种 或 奶酥馅" :disabled="isEditing" :class="{ 'is-disabled': isEditing }" />
					</FormItem>
					<FormItem label="配方类型">
				...
				<view class="card">
					...
					<view class="procedure-notes">
						...
						<AppButton type="dashed" full-width size="md" @click="addProcedureStep">+ 添加要点</AppButton>
					</view>
				</view>
		...
		<ExpandingFab v-if="form.type === 'PRE_DOUGH'" :icon="'/static/icons/calculator.svg'" @click="showCalculatorModal = true" :no-tab-bar="true" :visible="isFabVisible" />
	</view>
</template>
