	...
	<view class="calculator-container">
		<view class="form-container">
			<view v-if="calculationTargets.length > 1" class="form-item">
				...
				<picker mode="selector" :range="calculationTargets" range-key="name" @change="onPreDoughChange" :value="currentPreDoughIndex">
			...
			<view class="form-item">
				...
				<picker mode="selector" :range="availableBrands" range-key="label" @change="onBrandChange" :value="currentBrandIndex">
			...
			<view class="form-item">
				...
				<picker mode="selector" :range="availableTemperatures" @change="onTemperatureChange" :value="currentTempIndex">
			...
			<view class="form-item">
				...
				<picker mode="selector" :range="availableTimes" @change="onTimeChange" :value="currentTimeIndex" :disabled="!form.temperatureC">
		...
		<view v-if="result !== null || isLoading" class="result-card" :class="{ visible: result !== null || isLoading }">
			<view class="result-title">{{ flourWeightInGrams > 0 ? '建议用量 (克)' : '建议用量 (占总粉量)' }}</view>

			<view v-if="isLoading" class="result-value is-loading">计算中...</view>
...
const levainBrands = [{ label: '鲁邦种', value: 'LEVAIN' }];
...
const onPreDoughChange = (e: any) => {
	...
	if (selected.id === 'general') {
		...
		flourWeightInGrams.value = selectedPreDough.value.ingredients.filter((ing) => ing.name.includes('粉')).reduce((sum, ing) => sum + ing.weightInGrams, 0);
	}
