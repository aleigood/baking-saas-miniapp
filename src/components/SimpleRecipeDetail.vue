<template>
  <view class="card">
    <view class="meta-grid-container">
      ...
      <template v-if="recipeType === 'EXTRA'">
        <view class="meta-divider"></view>
        <view class="meta-item">
          <view class="label">分装余量</view>
          <view class="value">{{ divisionLossDisplay }}g/份</view>
        </view>
      </template>
      <view class="meta-divider"></view>
      <view class="meta-item">
    ...
    <view class="collapsible-content" :class="{ 'is-collapsed': isCollapsed }">
      <view class="smart-table">
        ...
        <view
          ...
          @click.stop="
            handleIconClick(
              ing.ingredient.extraInfo,
              'simple-ing-icon-' + ingIndex,
            )
          "
        >
          <view class="col-ingredient ingredient-name-cell">
            <view
              v-if="ing.ingredient.extraInfo"
              class="ingredient-with-icon"
              :id="'simple-ing-icon-' + ingIndex"
            >
              <view class="ingredient-name-wrapper">
                ...
                <text
                  class="recipe-tag"
                  v-if="
                    ing.ingredient.type === 'PRE_DOUGH' ||
                    ing.ingredient.type === 'EXTRA'
                  "
                  >自制</text
                >
              </view>
          ...
          <text class="col-ratio"
            >{{ toPercentage(ing.flourRatio ?? ing.ratio) }}%</text
          >
          <text class="col-price"></text>
        </view>
      </view>
...
const props = defineProps({
  ...
  recipeType: {
    type: String as PropType<"MAIN" | "PRE_DOUGH" | "EXTRA">,
    default: "PRE_DOUGH",
  },
});
...
const procedure = computed(() => {
  if (
    !props.version ||
    !props.version.components ||
    props.version.components.length === 0
  ) {
    return [];
  }
...
const nonMainRecipeIngredients = computed(() => {
  ...
  return component.ingredients.filter(
    (
      ing,
    ): ing is ComponentIngredient & {
      ingredient: NonNullable<ComponentIngredient["ingredient"]>;
    } => !!ing.ingredient,
  );
});
