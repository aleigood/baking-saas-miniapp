<template>
  ...
  <view class="page-wrapper" @click="hidePopover">
      ...
      >
        ...
        >
          ...
          <SimpleRecipeDetail
            ...
            :key="'simple-detail'"
            :version="renderedVersion"
            :recipe-type="recipeFamily.type"
            :usage-count="recipeFamily.usageCount || 0"
            @show-popover="handleShowPopover"
          />
        </view>
      </view>
    ...
    <AppModal
      ref="versionOptionsModalRef"
      v-model:visible="showVersionOptionsModal"
      :key="'version-options-modal'"
      title="配方版本"
      :no-header-line="true"
    >
      <view class="options-list">
        <ListItem
          v-if="!selectedVersionForAction?.isActive"
          class="option-item"
          @click="handleActivateVersionOption"
          :bleed="true"
        >
        ...
        <ListItem
          class="option-item"
          @click="handleEditVersionNotesOption"
          :bleed="true"
        >
          <view class="main-info">
    ...
    <AppModal
      v-model:visible="showEditVersionNotesModal"
      :key="'edit-version-notes-modal'"
      title="修改版本说明"
    >
      <FormItem label="版本说明">
        <input
          class="input-field"
          v-model="versionNotesDraft"
          maxlength="100"
          placeholder="请输入版本说明"
        />
      </FormItem>
      <view class="modal-actions">
        <AppButton
          type="secondary"
          @click="showEditVersionNotesModal = false"
          :disabled="isSubmitting"
          >取消</AppButton
        >
        <AppButton
          type="primary"
          @click="handleSaveVersionNotes"
          :loading="isSubmitting"
          :disabled="!versionNotesDraft.trim()"
          >保存</AppButton
        >
      </view>
    </AppModal>
    ...
    <AppModal
      v-model:visible="showActivateVersionConfirmModal"
      :key="'activate-confirm-modal'"
      title="设为使用中"
    >
      <view class="modal-prompt-text">要将这个版本设为当前使用的配方吗？</view>
      <view class="modal-warning-text"
        >后续创建生产任务时将默认使用此版本配方。</view
      >
      <view class="modal-actions">
        <AppButton
          type="secondary"
          @click="showActivateVersionConfirmModal = false"
          >取消</AppButton
        >
        <AppButton
          type="primary"
          @click="handleConfirmActivateVersion"
          :loading="isSubmitting"
        >
    ...
    <AppModal
      v-model:visible="showDependencyUpgradeModal"
      :key="'dependency-upgrade-modal'"
      title="更新关联配方"
    >
      ...
      <scroll-view :scroll-y="true" class="dependency-upgrade-scroll">
        <view
          v-if="pendingDependencyUpgradePlan?.dependencies.length"
          class="dependency-upgrade-list"
        >
          <view
            v-for="item in pendingDependencyUpgradePlan.dependencies"
            :key="item.familyId"
            class="dependency-upgrade-row"
          >
        ...
        <view v-else class="dependency-upgrade-list">
          <view
            v-for="item in dependencyUpgradePlan?.affectedRecipes || []"
            :key="item.familyId"
            class="dependency-upgrade-row"
          >
            <view class="dependency-recipe-copy">
              <text class="dependency-recipe-name">{{ item.familyName }}</text>
              <text class="dependency-recipe-type">{{
                getRecipeTypeLabel(item.type)
              }}</text>
            </view>
      ...
      <view class="modal-actions">
        <AppButton
          type="secondary"
          @click="showDependencyUpgradeModal = false"
          :disabled="isSubmitting"
          >暂不更新</AppButton
        >
        <AppButton
          type="primary"
          @click="handleApplyDependencyUpgrades"
          :loading="isSubmitting"
          >更新并使用</AppButton
        >
      </view>
    ...
    <AppModal
      v-model:visible="showOperationLogsModal"
      :key="'recipe-operation-logs-modal'"
      title="配方操作日志"
    >
      <scroll-view :scroll-y="true" class="operation-log-scroll">
        <view v-if="isLoadingOperationLogs" class="operation-log-empty"
          >正在加载...</view
        >
        <view v-else-if="operationLogs.length === 0" class="operation-log-empty"
          >暂无操作记录</view
        >
        <view v-else class="operation-timeline">
          ...
          >
            ...
            <view class="operation-log-content">
              ...
              <!-- 修改摘要展示为有颜色的标签 -->
              <view
                v-if="parseOperationLogTags(log).length > 0"
                class="log-change-tags"
              >
                <view
              ...
              <view class="operation-meta">
                <text class="operation-actor">{{
                  log.actor ? getUserDisplayName(log.actor) : "系统"
                }}</text>
                <text class="operation-meta-separator">·</text>
                <text class="operation-time">{{
    ...
    <AppPopover
      ...
      :target-rect="popover.targetRect"
      placement="right"
      :offsetY="0"
    />
...
const pendingDependencyUpgradePlan = ref<PendingDependencyUpgradePlan | null>(
  null,
);
...
const loadRecipeData = async (id: string) => {
  ...
  try {
    ...
    recipeVersions.value = fullFamilyData.versions.sort(
    ...
    let versionToShow =
      ...
      (recipeVersions.value.length > 0 ? recipeVersions.value[0] : null);

    if (versionToShow) {
      if (
        !displayedVersionId.value ||
        !recipeVersions.value.some((v) => v.id === displayedVersionId.value)
      ) {
    ...
    await Promise.all([
      loadDependencyUpgradePlan(
        id,
        currentActiveVersion?.id,
        fullFamilyData.deletedAt,
      ),
...
const loadPendingDependencyUpgradePlan = async (
  id: string,
  ...
  try {
    ...
    pendingDependencyUpgradePlan.value =
      plan.dependencies.length > 0 ? plan : null;
  } catch (error) {
    console.error("Failed to load pending dependency upgrades:", error);
  }
};

const loadDependencyUpgradePlan = async (
  id: string,
  versionId?: string,
  deletedAt?: string | null,
) => {
...
const displayedVersion = computed(() => {
  return (
    recipeVersions.value.find((v) => v.id === displayedVersionId.value) || null
  );
});

const renderedVersion = computed(() => {
  return (
    recipeVersions.value.find((v) => v.id === renderedVersionId.value) || null
  );
});
...
const currentUserRoleInTenant = computed(
  () =>
    userStore.userInfo?.tenants.find(
...
const canEditRecipe = computed(() => {
  return (
    (currentUserRoleInTenant.value === "OWNER" ||
      currentUserRoleInTenant.value === "ADMIN") &&
    recipeFamily.value?.readOnly !== true
  );
});
...
const navigateToEditPage = async (
  ...
  try {
    const formTemplate = await getRecipeVersionFormTemplate(
      familyId,
      versionId,
    );
    uni.setStorageSync(
      "source_recipe_version_form",
...
const handleSaveVersionNotes = async () => {
  if (
    !recipeFamily.value ||
    !selectedVersionForAction.value ||
