...
const getPrepTaskProgressStorageKey = (date: string) =>
  `prep-task-progress-${date}`;

export const useDataStore = defineStore("data", () => {
  const tenants = ref<Tenant[]>([]);
  const currentTenantId = ref<string>(uni.getStorageSync("tenant_id") || "");
  const production = ref<
    (ProductionTaskSummaryDto | (PrepTaskSummary & { status: "PREP" }))[]
  >([]);
  ...
  // [新增] 加载指定日期的前置任务进度
  const loadPrepTaskProgress = (
    date: string,
  ): { addedIngredients: Set<string>; completedItems: Set<string> } => {
    ...
    if (savedData) {
      ...
      try {
        ...
        const addedIngredients = Array.isArray(parsed.addedIngredients)
          ? new Set<string>(parsed.addedIngredients)
          : new Set<string>();
        const completedItems = Array.isArray(parsed.completedItems)
          ? new Set<string>(parsed.completedItems)
          : new Set<string>();
    ...
    return {
      addedIngredients: new Set<string>(),
      completedItems: new Set<string>(),
    };
  };

  // [新增] 保存指定日期的前置任务进度
  const savePrepTaskProgress = (
    date: string,
  ...
  const completeProductionTaskLocally = (taskId: string) => {
    const taskIndex = production.value.findIndex(
      (task) => task.status !== "PREP" && task.id === taskId,
    );
    ...
    homeStats.value.pendingCount = Math.max(
      0,
      homeStats.value.pendingCount - completedQuantity,
    );
  };
  ...
  async function fetchTenants() {
    try {
      ...
      const userTenants =
        userStore.userInfo?.tenants.map((t) => t.tenant) || [];
      tenants.value = userTenants;

      if (userTenants.length > 0) {
        const storedIdIsValid = userTenants.some(
  ...
  async function fetchHistoricalTasks(loadMore = false) {
    ...
    if (
      !loadMore &&
      !dataStale.historicalTasks &&
      dataLoaded.value.historicalTasks
    ) {
      return;
    }
    ...
    try {
      ...
      const res = await getHistoryTasks(
        pageToFetch,
        historicalTasksMeta.value.limit,
      );
  ...
  async function fetchRecipesData() {
    ...
    try {
      ...
      const [recipeData, recipeStatData] = await Promise.all([
        getRecipes(),
        getRecipeStats(startDate, endDate),
      ]);
  ...
  async function fetchMembersData() {
    ...
    try {
      const currentUserRole = userStore.userInfo?.tenants.find(
        (t) => t.tenant.id === currentTenantId.value,
      )?.role;

      if (currentUserRole === "OWNER") {
        const allTenantsWithMembers = await getAllMembersByOwner();
        const currentTenantMembers = allTenantsWithMembers.find(
          (t) => t.tenantId === currentTenantId.value,
        );
        members.value = currentTenantMembers
          ? currentTenantMembers.members
          : [];
      } else {
  ...
  function resetData() {
    ...
    Object.keys(dataStale).forEach(
      (key) => (dataStale[key as keyof typeof dataStale] = true),
    );
    resetHistoricalTasks();
  }
