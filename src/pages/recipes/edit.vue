<template>
  ...
  <view class="page-wrapper" @click="handlePageClick">
    ...
    <DetailPageLayout @scroll="handleScroll">
      <view class="page-content page-content-with-fab">
        ...
        <view class="card" v-if="form.type === 'MAIN'" :key="'params-main'">
          ...
          <FormItem label="工艺损耗率 (%)">
            <input
              class="input-field"
              type="digit"
              v-model="mainComponent.lossRatio"
              placeholder="例如: 1"
            />
          </FormItem>
          <FormItem label="分割余量 (g/个)">
            <input
              class="input-field"
              type="digit"
              v-model="mainComponent.divisionLoss"
              placeholder="例如: 2"
            />
          </FormItem>
        </view>
        ...
        <view class="card" v-if="form.type !== 'MAIN'" :key="'params-other'">
          ...
          <FormItem v-if="form.type === 'EXTRA'" label="分装余量 (g/份)">
            <input
              class="input-field"
              type="digit"
              v-model="mainComponent.divisionLoss"
        ...
        <view class="card" v-if="form.type === 'MAIN'" :key="'details-main'">
                ...
                >
                  ...
                  <view class="table-wrapper">
                    <view class="smart-table read-only-table">
                      ...
                      <view
                        v-for="ing in component.ingredients"
                        :key="ing.name"
                        class="table-row"
                      >
                        <view class="col-name">
                          <view class="ingredient-name-wrapper">
                            <text>{{ ing.name }}</text>
                            <text v-if="ing.isFlour" class="mini-tag flour-tag"
                              >面粉</text
                            >
                          </view>
                        </view>
                        <text class="col-ratio"
                          >{{ formatNumber(ing.ratio) }}%</text
                        >
                      </view>
                    </view>
                  </view>
                  <view
                    ...
                    class="procedure-notes-read-only"
                  >
          ...
          <view>
            ...
            <view
              v-for="(ing, ingIndex) in mainComponent.ingredients"
              :key="ing._key || ingIndex"
            ...
            >
              <view class="autocomplete-input-wrapper">
                <AutocompleteInput
                  ...
                  @blur="
                    handleIngredientBlur(ing, availableMainDoughIngredients)
                  "
                  :tags="getIngredientTags(ing, true)"
                  :creationOptions="
                    showFlourCheckbox
                      ? [{ label: '新建面粉', payload: { isFlour: true } }]
                      : []
                  "
                >
                  <template #item="{ item }">
                    <view class="suggestion-row-wrapper">
                      ...
                      <view class="suggestion-tags-wrapper">
                        <text v-if="item.isFlour" class="mini-tag flour-tag"
                          >面粉</text
                        >
                        <text v-if="item.isRecipe" class="mini-tag recipe-tag"
                          >自制</text
                        >
                      </view>
                    </view>
          ...
          <view class="procedure-notes">
            <text class="notes-title">制作要点</text>
            <view
              v-for="(step, stepIndex) in mainComponent.procedure"
              :key="stepIndex"
              class="procedure-item"
            >
              <input
                class="input-field"
                v-model="mainComponent.procedure[stepIndex]"
                placeholder="输入制作步骤"
              />
              <IconButton
                variant="field"
                @click="removeProcedureStep(mainComponent, stepIndex)"
              >
                <image class="remove-icon" src="/static/icons/trash.svg" />
              </IconButton>
            </view>
            <AppButton
              ...
              full-width
              size="md"
              @click="addProcedureStep(mainComponent)"
              >+ 添加制作要点</AppButton
            >
          </view>
        </view>
        ...
        <template v-if="form.type === 'MAIN'">
          ...
          <view
            v-for="(product, prodIndex) in form.products"
            :key="product._key || prodIndex"
          >
            <view
              ...
              :class="{
                'is-fading-out': isFadingOutProduct,
                'product-edit-card-animate': isAnimatingProduct,
              }"
            >
              <view class="card-title-wrapper">
                <span class="card-title">{{
                  product.name || `产品${prodIndex + 1}`
                }}</span>
              ...
              <FormItem label="基础原料克重 (g)">
                <input
                  class="input-field"
                  type="digit"
                  v-model="product.baseDoughWeight"
                  placeholder="例如: 100"
                />
              </FormItem>
                ...
                >
                  <view class="autocomplete-input-wrapper">
                    ...
                    >
                      <template #item="{ item }">
                        <view class="suggestion-row-wrapper">
                          <view class="suggestion-name-text">{{
                            item.name
                          }}</view>
                          <view class="suggestion-tags-wrapper">
                            <text v-if="item.isFlour" class="mini-tag flour-tag"
              ...
              <view class="sub-group">
                ...
                >
                  ...
                  <input
                    class="input-field ratio-input"
                    type="digit"
                    v-model="ing.weightInGrams"
                    placeholder="g/个"
                  />
                  <IconButton
                    variant="field"
                    @click="
                      removeSubIngredient(prodIndex, 'fillings', ingIndex)
                    "
                  >
                    <image class="remove-icon" src="/static/icons/trash.svg" />
                  </IconButton>
              ...
              <view class="sub-group">
                ...
                >
                  ...
                  <IconButton
                    variant="field"
                    @click="
                      removeSubIngredient(prodIndex, 'toppings', ingIndex)
                    "
                  >
                    <image class="remove-icon" src="/static/icons/trash.svg" />
                  </IconButton>
              ...
              <view class="procedure-notes">
                ...
                >
                  ...
                  <IconButton
                    variant="field"
                    @click="removeProcedureStep(product, stepIndex)"
                  >
                    <image class="remove-icon" src="/static/icons/trash.svg" />
                  </IconButton>
                </view>
                <AppButton
                  ...
                  full-width
                  size="md"
                  @click="addProcedureStep(product)"
                  >+ 添加制作要点</AppButton
                >
              </view>
            </view>
    ...
    <AppModal
      :visible="showAddPreDoughModal"
      :key="'add-predough-modal'"
      @update:visible="showAddPreDoughModal = false"
      title="添加面种"
    >
      <FormItem label="选择面种配方">
        <picker
          mode="selector"
          :range="availablePreDoughs"
          range-key="name"
          @change="onPreDoughSelect"
        >
      ...
      <FormItem label="面种用粉占比 (%)">
        <input
          class="input-field"
          type="digit"
          v-model="preDoughFlourRatio"
          placeholder="例如：20"
        />
      </FormItem>
      <view class="modal-actions">
        <AppButton type="secondary" @click="showAddPreDoughModal = false"
          >取消</AppButton
        >
        <AppButton
          type="primary"
          @click="confirmAddPreDough"
          :loading="isAddingPreDough"
        >
          {{ isAddingPreDough ? "..." : "确认" }}
        </AppButton>
      </view>
    ...
    <AppModal
      v-model:visible="showCalculatorModal"
    ...
    <ExpandingFab
      ...
      :no-tab-bar="true"
      :visible="isFabVisible && showFermentationCalculator"
    />
  </view>
</template>
...
const form = ref<
  Omit<
    RecipeFormTemplate,
    "ingredients" | "procedure" | "products" | "components"
  > & {
...
const showFermentationCalculator = computed(() => {
  ...
  const isBreadProduct =
    form.value.type === "MAIN" && form.value.category === "BREAD";
...
const currentCategoryLabel = computed(() => {
  return (
    recipeCategories.value.find((c) => c.value === form.value.category)
      ?.label || "请选择"
  );
});
...
const currentTypeLabel = computed(() => {
  return (
    recipeTypes.value.find((t) => t.value === form.value.type)?.label ||
    "请选择"
  );
});

const onCategoryChange = (e: any) => {
  form.value.category = recipeCategories.value[e.detail.value]
    .value as RecipeCategory;
};

const onTypeChange = (e: any) => {
  form.value.type = recipeTypes.value[e.detail.value].value as
    | "PRE_DOUGH"
    | "EXTRA";
};
...
const mainComponent = computed(
  () =>
    form.value.components!.find(
      (c) => c.type === "MAIN_DOUGH" || c.type === "BASE_COMPONENT",
    )!,
);
...
const availablePreDoughs = computed(() => {
  const existingPreDoughIds = new Set(
    form.value.components
      ?.filter((c) => c.type === "PRE_DOUGH")
      .map((c) => c.id),
  );
  ...
  return (dataStore.recipes.preDoughs || []).filter(
    (r) =>
      r.activeVersion &&
      !r.deletedAt &&
      !existingPreDoughIds.has(r.id) &&
      r.name !== currentRecipeName,
  );
});
...
const availableMainDoughIngredients = computed((): AutocompleteItem[] => {
  ...
  ingredientPresets.value.forEach((p) => {
    ingredientMap.set(p.name, {
      id: null,
      name: p.name,
      isFlour: p.isFlour,
      isRecipe: false,
      waterContent: p.waterContent || 0,
      recipeType: null,
    });
  });
  dataStore.allIngredients.forEach((i) => {
    ingredientMap.set(i.name, {
      id: i.id,
      name: i.name,
      isFlour: i.isFlour,
      isRecipe: false,
      waterContent: i.waterContent || 0,
      recipeType: null,
    });
  });
  ...
  if (currentFormType === "MAIN") {
    ...
    extras.forEach((e) => {
      if (e.activeVersion && !e.deletedAt && e.name !== currentRecipeName) {
        ...
        ingredientMap.set(e.name, {
          id: e.id,
          name: e.name,
          isFlour: false,
          isRecipe: true,
          waterContent: effectiveWaterContent,
          recipeType: "EXTRA",
          recipeVersionId: e.activeVersion?.id,
        });
      }
    });
    ...
    preDoughs.forEach((p) => {
      if (p.activeVersion && !p.deletedAt && p.name !== currentRecipeName) {
        ...
        ingredientMap.set(p.name, {
          ...
          recipeVersionId: p.activeVersion?.id,
        });
      }
    });
  ...
  if (currentFormType === "MAIN") {
    const preDoughNames = new Set(
      (dataStore.recipes.preDoughs || [])
        .filter((r) => !r.deletedAt)
        .map((r) => r.name),
    );
    return combined
      .filter((item) => !preDoughNames.has(item.name))
...
const calculatedWaterContentPreview = computed(() => {
  ...
  component.ingredients.forEach((ing) => {
    ...
    const item = availableMainDoughIngredients.value.find(
      (i) => i.name === ing.name,
    );
...
const manualWaterRatio = computed(() => {
  ...
  const targetIngredient = mainComponent.value.ingredients.find(
    (i) => i.name === targetName,
  );
...
const showTotalWaterTag = computed(() => {
  ...
  const diff = Math.abs(
    totalCalculatedWaterRatio.value - explicitWaterRatio.value,
  );
  return diff > 0.1;
});
...
const getIngredientTags = (
  ing: MainIngredient | SubIngredientWeight | SubIngredientRatio,
  isMainDough: boolean = false,
) => {
  const tags = [];
  if (ing.isFlour) {
    tags.push({
      text: "面粉",
      style: { backgroundColor: "#ebe2d9", color: "#8d6e63" },
    });
  }
  ...
  if (ing.isRecipe) {
    tags.push({
      text: "自制",
      style: { backgroundColor: "#faedcd", color: "var(--primary-color)" },
    });
  }

  if (
    isMainDough &&
    waterTagTargetName.value &&
    ing.name === waterTagTargetName.value &&
    showTotalWaterTag.value
  ) {
    tags.push({
...
const handleIngredientBlur = (
  ...
  if (!ingredient.id && ingredient.name) {
    const existing = availableList.find(
  ...
  if (options && options.familyId) {
    ...
    if (sourceFormJson) {
      try {
        ...
        const sanitizedComponents = parsedForm.components.map(
          (c: ComponentTemplate) => ({
            ...c,
            ingredients: c.ingredients.map((ing: any) => ({
              ...
              flourRatio:
                ing.isRecipe &&
                (parsedForm.type === "PRE_DOUGH" ||
                  (parsedForm.type === "MAIN" && ing.ratio === null))
                  ? ing.ratio
                  : null,
              ratio:
                ing.isRecipe &&
                (parsedForm.type === "PRE_DOUGH" ||
                  ...
                  ? null
                  : ing.ratio,
              recipeType: ing.isRecipe
                ? availablePreDoughs.value.some((p) => p.id === ing.id)
                  ? "PRE_DOUGH"
                  : "EXTRA"
                : null,
            })),
        ...
        form.value = {
          ...parsedForm,
          customWaterContent:
            parsedForm.components[0]?.customWaterContent !== undefined &&
            parsedForm.components[0]?.customWaterContent !== null
          ...
          products: (parsedForm.products || []).map((p: any) => ({
            ...
            mixIns: (p.mixIns || []).map((i: any) => ({
              ...i,
              _key: i.id || "sub_" + Math.random().toString(36).substr(2, 9),
            })),
            fillings: (p.fillings || []).map((i: any) => ({
              ...i,
              _key: i.id || "sub_" + Math.random().toString(36).substr(2, 9),
            })),
            toppings: (p.toppings || []).map((i: any) => ({
              ...i,
              _key: i.id || "sub_" + Math.random().toString(36).substr(2, 9),
            })),
          })),
          ...
          form.value.components.forEach((comp) => {
            comp.ingredients.forEach((ing) => {
              if (ing.isRecipe && ing.id) {
                const found = availableMainDoughIngredients.value.find(
                  (item) => item.id === ing.id,
                );
                if (found) {
...
const onIngredientSelect = (
  item: AutocompleteItem & { isFlour?: boolean },
  ingIndex: number,
) => {
...
const addIngredient = () => {
  mainComponent.value.ingredients.push({
    id: null,
    _key: "ing_" + Math.random().toString(36).substr(2, 9),
    name: "",
    ratio: null,
    flourRatio: null,
    isFlour: false,
...
const handlePreDoughRatioChange = (component: EnhancedComponent) => {
  ...
  const originalFlourSum = component._originalIngredients.reduce(
    (sum, ing) => sum + (ing.isFlour ? ing.ratio || 0 : 0),
    0,
  );
...
const confirmAddPreDough = async () => {
  if (
    !selectedPreDough.value ||
    !preDoughFlourRatio.value ||
    preDoughFlourRatio.value <= 0
  ) {
    toastStore.show({
      message: "请选择面种并填写有效的面粉占比",
      type: "error",
    });
    return;
  }
  ...
  try {
    ...
    if (
      !activeVersion ||
      !activeVersion.components ||
      activeVersion.components.length === 0
    ) {
      toastStore.show({
        message: "所选面种没有使用中的版本，无法添加",
        type: "error",
      });
      return;
    }
    ...
    const preDoughInternalFlourRatio = ingredients.reduce((sum, ingredient) => {
      ...
      if (ingredient.ingredient?.type === "PRE_DOUGH")
        return sum + (ingredient.flourRatio ?? 0);
      return sum;
    }, 0);

    if (preDoughInternalFlourRatio <= 0) {
      toastStore.show({
        message: "所选面种配方中不含面粉，无法添加",
        type: "error",
      });
      return;
    }

    const targetFlourRatioInMainDoughDecimal = toDecimal(
      preDoughFlourRatio.value,
    );
    const scalingFactor =
      targetFlourRatioInMainDoughDecimal / preDoughInternalFlourRatio;

    const displayIngredients: MainIngredient[] = ingredients.map(
      (ingredient) => {
        const displayIngredient = ingredient.ingredient!;
        const recipeType =
          displayIngredient.type === "PRE_DOUGH" ||
          displayIngredient.type === "EXTRA"
            ? displayIngredient.type
            : null;
        const sourceRatio =
          recipeType === "PRE_DOUGH" ? ingredient.flourRatio : ingredient.ratio;
        return {
          ...
          recipeType,
          recipeVersionId:
            ingredient.preDoughVersionId ??
            ingredient.extraVersionId ??
            undefined,
        };
      },
    );
...
const addSubIngredient = (
  ...
  if (type === "mixIns") {
    ...
    product.mixIns.push({
      id: null,
      _key: "sub_" + Math.random().toString(36).substr(2, 9),
      name: "",
      ratio: null,
      weightInGrams: null,
      isRecipe: false,
...
const removeProcedureStep = (
  itemWithProcedure: { procedure: string[] },
  index: number,
) => {
...
const handleSubmit = async () => {
  ...
  try {
    const mainComponentFromForm = form.value.components!.find(
      (c) => c.type === "MAIN_DOUGH" || c.type === "BASE_COMPONENT",
    );
    ...
    const ingredientsPayload = mainComponentFromForm.ingredients
      ...
      .map((ing) => {
        if (ing.recipeType === "PRE_DOUGH") {
          if (ing.flourRatio === null || Number(ing.flourRatio) <= 0)
            return null;
          return {
            name: ing.name,
    ...
    const processSubIngredients = (
      subIngredients: (SubIngredientWeight | SubIngredientRatio)[],
      type: "MIX_IN" | "FILLING" | "TOPPING",
    ) => {
      return subIngredients
        .filter(
          (i) =>
            i.name &&
            (("ratio" in i && i.ratio !== null && Number(i.ratio) > 0) ||
              ("weightInGrams" in i &&
                i.weightInGrams !== null &&
                Number(i.weightInGrams) > 0)),
        )
        .map((i) => {
          const base: {
            name: string;
          ...
          } = { name: i.name, type, recipeVersionId: i.recipeVersionId };
          if (i.id) {
    ...
    const finalCustomWaterContent =
      currentCustom !== null && currentCustom !== undefined
        ? Number(currentCustom)
        : null;
    ...
    const payload = {
      ...
      targetTemp:
        form.value.type === "MAIN" && form.value.category === "BREAD"
          ? toOptionalNumber(form.value.targetTemp)
          : null,
